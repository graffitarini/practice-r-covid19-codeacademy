...
# Load the data
confirmed <- read.csv("/Users/novegagraffitarini/Downloads/CoronavirusProject/corona_confirmed.csv")
deaths <- read.csv("/Users/novegagraffitarini/Downloads/CoronavirusProject/corona_deaths.csv")
recovered <- read.csv("/Users/novegagraffitarini/Downloads/CoronavirusProject/corona_recovered.csv")
...
australia_cases
# Display percentages
australia_percentage <- australia_cases / total_cases * 100
north_percentage <- cases_in_north / total_cases * 100
...
australia_confirmed <- countries_confirmed %>%
  filter(`Country.Region` == 'Australia') %>%
  select(`X3.22.20`) %>%
  sum()
australia_confirmed
...
```{r}
# Group by countries
us_confirmed <- countries_confirmed %>%
...
us_percentage <- us_confirmed / total_cases * 100
us_percentage

us_recovered_percent <- us_recovered / total_cases * 100
us_recovered_percent
```
...
countries_confirmed_t <- countries_confirmed %>%
  t() %>%
  as.data.frame()

countries_confirmed_t %>%
  head()
```
...
countries_confirmed_t <- apply(countries_confirmed_t, 2) %>% as.data.frame()
head(countries_confirmed_t)
```
...
us_max <- countries_confirmed_t$US %>%
  max()
...
transposed_confirmed <- countries_confirmed_t %>% mutate(date = c(1:nrow(countries_confirmed_t)))
...
#transposed_confirmed %>% ggplot(aes(x = date, y = Italy)) + geom_line() + after_stat(x)()

italy_case <- transposed_confirmed %>% ggplot(aes(x = date, y = Italy)) + geom_line()
italy_case
```
